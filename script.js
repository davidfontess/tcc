	...
	function preprocessarCodigo(codigo) {
		// remove declarações const/let/var mantendo o padrão original
		return codigo.replace(/(const|let|var)\s+(\w+)\s*=/g, '$2 =');
	}
...
desenhar(circ); // Desenha o circulo no canvas`);
	...
	function executarCodigo() {
		...
		try {
			...
			const codigoProcessado = preprocessarCodigo(codigo);
			const funcaoUsuario = new Function(codigoProcessado);
